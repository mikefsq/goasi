...
func matchEFW(d linuxDev) bool {
	return d.vid == VID && (d.pid == PIDEFW1 || d.pid == PIDEFW2)
}
...
func openLinux(d linuxDev) (Transport, DeviceInfo, error) {
	...
	// hidraw does not expose the report length cheaply; the EFW uses 64-byte
	// reports (newEFW normalizes anyway).
	info := DeviceInfo{PID: d.pid, LocationID: d.loc, FeatureLen: 64}
	return &linuxTransport{f: f}, info, nil
}

// Enumerate lists all attached ZWO EFW hidraw devices.
...
func openFirst() (Transport, DeviceInfo, error) {
	...
	return nil, DeviceInfo{}, errors.New("no ZWO EFW found in /sys/class/hidraw")
}

// OpenLocation opens the EFW at a specific hidraw location (from Enumerate).
func OpenLocation(loc uint32) (Transport, DeviceInfo, error) {
	for _, d := range scanHidraw() {
		if matchEFW(d) && d.loc == loc {
			return openLinux(d)
		}
	}
	return nil, DeviceInfo{}, fmt.Errorf("no EFW at hidraw location %d", loc)
}
