...
func enumerateWindows() ([]winDev, error) {
	...
	for i := 0; ; i++ {
		...
		if !ok || vid != VID || (pid != PIDEFW1 && pid != PIDEFW2) {
			continue
		}
...
// Enumerate lists all attached ZWO EFW HID devices.
...
func openFirst() (Transport, DeviceInfo, error) {
	...
	if len(devs) == 0 {
		return nil, DeviceInfo{}, errors.New("no ZWO EFW found")
	}
	return openWindows(devs[0])
}

// OpenLocation opens the EFW whose path hashes to loc (from Enumerate).
func OpenLocation(loc uint32) (Transport, DeviceInfo, error) {
	...
	return nil, DeviceInfo{}, fmt.Errorf("no EFW at location 0x%08x", loc)
}
