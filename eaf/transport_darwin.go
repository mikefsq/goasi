...
// eaf_enumerate lists all HID devices matching VID and either PID, filling
// outLoc[]/outPid[] (up to maxN entries). Returns the count, or -1 on failure.
static int eaf_enumerate(uint32_t vid, uint32_t pid1, uint32_t pid2,
...
// eaf_open finds a HID device with the given VID and either PID. If wantLoc != 0
...
static void* eaf_open(uint32_t vid, uint32_t pid1, uint32_t pid2, uint32_t wantLoc,
...
static int eaf_setfeature(void* dev, uint8_t* buf, int len) {
    if (len < 1) return -1;
...
static int eaf_getfeature(void* dev, uint8_t* buf, int len) {
    if (len < 1) return -1;
...
static void eaf_close(void* dev) {
    if (dev) {
...
// eaf_debug_list enumerates ALL HID devices (no matching filter) and prints
...
static int eaf_debug_list(void) {
    IOHIDManagerRef mgr = IOHIDManagerCreate(kCFAllocatorDefault, kIOHIDOptionsTypeNone);
    if (!mgr) return -2;
...
func DebugListAll() int { return int(C.eaf_debug_list()) }

type iokitTransport struct{ dev unsafe.Pointer }

// Enumerate lists all attached ZWO EAF HID devices (by USB locationID + PID).
// The ZWO factory serial is NOT in the USB layer, so callers that need it must
// open each device and query (see eaf.List).
func Enumerate() ([]DeviceInfo, error) {
	const maxN = 32
	var locs [maxN]C.uint32_t
	var pids [maxN]C.uint32_t
	n := int(C.eaf_enumerate(C.uint32_t(VID), C.uint32_t(PIDEAF1), C.uint32_t(PIDEAF2), &locs[0], &pids[0], maxN))
	if n < 0 {
...
func openDev(wantLoc uint32) (Transport, DeviceInfo, error) {
	...
	var loc C.uint32_t
	dev := C.eaf_open(
		C.uint32_t(VID), C.uint32_t(PIDEAF1), C.uint32_t(PIDEAF2), C.uint32_t(wantLoc),
		&serial[0], 256, &product[0], 256, &maxFeat, &loc,
	)
	if dev == nil {
		return nil, DeviceInfo{}, errors.New("no matching ZWO EAF (none attached, another process holds it, or HID access denied)")
	}
	info := DeviceInfo{
...
// OpenLocation opens the EAF at a specific USB locationID (from Enumerate).
...
func (t *iokitTransport) SetFeature(buf []byte) error {
	...
	if rc := C.eaf_setfeature(t.dev, (*C.uint8_t)(unsafe.Pointer(&buf[0])), C.int(len(buf))); rc != 0 {
...
func (t *iokitTransport) GetFeature(buf []byte) error {
	...
	if rc := C.eaf_getfeature(t.dev, (*C.uint8_t)(unsafe.Pointer(&buf[0])), C.int(len(buf))); rc < 0 {
